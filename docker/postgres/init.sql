...
CREATE TYPE track_status_enum AS ENUM ('pending', 'scheduled', 'playing', 'played', 'failed');
...
CREATE OR REPLACE FUNCTION normalize_artist_name(raw_name TEXT)
RETURNS TEXT AS $$
BEGIN
    RETURN lower(trim(raw_name));
END;
$$ LANGUAGE plpgsql;
...
DECLARE
    ...
    raw_string TEXT;
BEGIN
    raw_string := p_artist_string;
    
    -- Определяем featured артистов (через "feat.", "ft.", "featuring")
    IF raw_string ~* 'feat\.|ft\.|featuring' THEN
        -- Разделяем основного и приглашенного артиста
        ...
        FOREACH artist_name IN ARRAY main_artists
        LOOP
            artist_name := trim(artist_name);
            CONTINUE WHEN artist_name = '';
            
            INSERT INTO artists (name, normalized_name)
            VALUES (artist_name, normalize_artist_name(artist_name))
        ...
        FOREACH artist_name IN ARRAY featured_artists
        LOOP
            artist_name := trim(artist_name);
            CONTINUE WHEN artist_name = '';
        ...
        END LOOP;
    ELSE
        -- Стандартный парсинг для остальных случаев
        artist_names := string_to_array(raw_string, '&');
        IF array_length(artist_names, 1) = 1 THEN
            artist_names := string_to_array(raw_string, ',');
        END IF;
...
CREATE OR REPLACE FUNCTION update_track_counters()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE tracks 
...
CREATE TRIGGER track_play_trigger
    ...
    FOR EACH ROW EXECUTE FUNCTION update_track_counters();

-- Ограничение очереди (максимум 10 треков)
CREATE OR REPLACE FUNCTION check_queue_limit()
RETURNS TRIGGER AS $$
DECLARE
    queue_count INTEGER;
BEGIN
    -- Считаем только треки в статусе pending
    SELECT COUNT(*) INTO queue_count 
    FROM track_queue 
    WHERE status = 'pending';
    
    -- Если добавляется новый pending трек, проверяем лимит
    IF NEW.status = 'pending' AND queue_count >= 10 THEN
        RAISE EXCEPTION 'Queue limit exceeded (max 10 pending tracks)';
    END IF;
...
CREATE TRIGGER queue_limit_trigger
    BEFORE INSERT OR UPDATE ON track_queue
    FOR EACH ROW EXECUTE FUNCTION check_queue_limit();
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN tr.rating = 1 THEN tr.id END) as actual_likes,
    COUNT(DISTINCT CASE WHEN tr.rating = -1 THEN tr.id END) as actual_dislikes
FROM tracks t
LEFT JOIN track_artists ta ON t.id = ta.track_id
LEFT JOIN artists a ON ta.artist_id = a.id
LEFT JOIN play_history ph ON t.id = ph.track_id
LEFT JOIN track_ratings tr ON t.id = tr.track_id
GROUP BY t.id, t.title, t.duration, t.total_plays, t.like_count, t.dislike_count;
